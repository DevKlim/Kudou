...
This phase is purely about UI implementation. The MCP's focus should be on understanding how to create and register a new editor dock.

### Key Files for This Phase:

*   **Files to be Created:**
    *   `editor/ai/agent_dock.h` & `.cpp`: Will define the `AgentDock` class, its UI elements (`RichTextLabel`, `LineEdit`), and basic layout.
    *   `editor/ai/SCsub`: The SCons build script to compile the new `ai` directory.

*   **Files to be Modified (for integration):**
    *   `editor/editor_node.h` & `.cpp`: The main editor class. The MCP will need to read this to understand how other docks are initialized and add the new `AgentDock` to the editor's UI hierarchy.
